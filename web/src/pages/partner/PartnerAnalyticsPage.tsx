 ...
 * 
 * Self-service analytics dashboard for partners
 */
...
const PartnerAnalyticsPage: React.FC = () => {
  ...
  const loadAnalytics = async () => {
    ...
    setLoading(true);
    try {
      const data = await webAnalyticsService.getPartnerAnalytics(user.id, startDate, endDate);
      setAnalytics(data);
    } catch (error) {
      logger.error('Failed to load partner analytics', error as Error);
    } finally {
      setLoading(false);
    }
  };
        ...
        <Grid container spacing={3} sx={{ mb: 3 }}>
          <Grid item xs={12} sm={6} md={3}>
            <KPICard
              ...
              subtitle={`${analytics.paidPromotionViews || 0} from promotions`}
          ...
          <Grid item xs={12} sm={6} md={3}>
            <KPICard
              ...
              value={analytics.feedbackRating.toFixed(1)}
              subtitle={`${analytics.feedbackCount} reviews`}
              icon={<StarOutlinedIcon sx={{ fontSize: 40 }} />}
              color="#9c27b0"
            />
          </Grid>
        </Grid>
